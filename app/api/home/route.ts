...
export async function GET() {
  try {
    ...
    const upcomingCards: HomeMovieCardItem[] = upcomingMovies
    ...
    const topRatedCards: HomeMovieCardItem[] = (Array.isArray(topRatedMovies) ? topRatedMovies : [])
      .filter((movie: any) => movie.poster_path)
      .map((movie: any, index: number) => ({
    ...
    return NextResponse.json(
      { boxOfficeCards, upcomingCards, topRatedCards },
