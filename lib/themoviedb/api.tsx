...
export async function getTodayMovies () {
  //한달내 업데이트된 영화
  const URL = `https://api.themoviedb.org/3/discover/movie?release_date.gte=${month}&release_date.lte=${today}&language=ko&watch_region=KR&with_watch_providers=8|9|96|97|337|350|356&without_watch_providers=1796&sort_by=release_date.desc&api_key=${API_KEY}`
  const response = await fetch(URL)
  const {results} = await response.json()
  return results
}
...
export async function getTodaySeries () {
  //오늘 업데이트된 시리즈
  // const URL = `https://api.themoviedb.org/3/discover/tv?air_date.gte=${today}&air_date.lte=${today}&language=ko&watch_region=KR&with_watch_providers=8|9|96|97|337|350|356&without_watch_providers=1796&without_genres=16&sort_by=first_air_date.desc&api_key=${API_KEY}`
  //한달내 첫방송 시작한 시리즈
  const URL = `https://api.themoviedb.org/3/discover/tv?first_air_date.gte=${month}&first_air_date.lte=${today}&language=ko&watch_region=KR&with_watch_providers=8|9|96|97|337|350|356&without_watch_providers=1796&sort_by=first_air_date.desc&without_genres=16&api_key=${API_KEY}`
  const response = await fetch(URL)
  const {results} = await response.json()
  return results
}
...
export async function getMonthAnime() {
  const URL = `https://api.themoviedb.org/3/discover/tv?air_date.gte=${month}&air_date.lte=${today}&language=ko&watch_region=KR&with_watch_providers=8|9|96|97|337|350|356&without_watch_providers=1796&with_genres=16&sort_by=first_air_date.desc&without_genres=10762&api_key=${API_KEY}`
  const response = await fetch(URL)
  const {results} = await response.json()
  return results
}
